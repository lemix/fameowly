...
function buildApiMessages(msgs: MessageData[]) {
  return msgs.map((m) => ({
    id: m.id,
    role: m.role,
    content: m.content,
  ...
  switch (event.type) {
    case "text-delta":
      ...
      setMessages((prev) => {
        ...
        if (last?.role === "assistant") {
          updated[updated.length - 1] = { ...last, content: acc.text };
        }
        return updated;
      });
      break;
    case "reasoning-delta":
      ...
      setMessages((prev) => {
        ...
        const last = updated[updated.length - 1];
        if (last?.role === "assistant") {
          updated[updated.length - 1] = { ...last, reasoning: acc.reasoning };
        }
        return updated;
      });
      break;
...
export function useChatStreaming(params: UseChatStreamingParams) {
      ...
      try {
        const response = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            ...
            ...(model.provider === "local" && localOptions
              ? { temperature: localOptions.temperature, reasoningEnabled: localOptions.reasoningEnabled }
              : {}),
          }),
        ...
        if (acc.error) {
          ...
          setMessages((prev) => {
            ...
            const last = updated[updated.length - 1];
            if (last?.role === "assistant") {
              updated[updated.length - 1] = { ...last, error: acc.error };
            }
            return updated;
          });
          setStatus("error");
  ...
  const retry = useCallback(
    (model: ModelOption, localOptions?: { temperature?: number; reasoningEnabled?: boolean }) => {
      setError(null);
